...
# No backref import needed if not used, same for text if not used directly.

from pytoniq import LiteBalancer # Assuming this is still needed for deposits
import asyncio
...
class User(Base):
    ...
    referrer = relationship("User", remote_side=[id], foreign_keys=[referred_by_id], backref="referrals_made_explicit", uselist=False) # changed backref name
    # referrals_made is implicitly created by referrer's backref if not defined here.
    # If you need to query User.referrals_made, ensure backref creates it or define it explicitly.
...
cases_data_backend_with_fixed_prices = [ { 'id': 'lolpop', 'name': 'Lol Pop Stash', 'priceTON': 1.5, 'prizes': [ { 'name': 'Plush Pepe', 'probability': 0.001 }, { 'name': 'Neko Helmet', 'probability': 0.005 }, { 'name': 'Party Sparkler', 'probability': 0.07 }, { 'name': 'Homemade Cake', 'probability': 0.07 }, { 'name': 'Cookie Heart', 'probability': 0.07 }, { 'name': 'Jack-in-the-box', 'probability': 0.06 }, { 'name': 'Skull Flower', 'probability': 0.023 }, { 'name': 'Lol Pop', 'probability': 0.25 }, { 'name': 'Hynpo Lollipop', 'probability': 0.25 }, { 'name': 'Desk Calendar', 'probability': 0.10 }, { 'name': 'B-Day Candle', 'probability': 0.101 } ] }, { 'id': 'recordplayer', 'name': 'Record Player Vault', 'priceTON': 6.0, 'prizes': [ { 'name': 'Plush Pepe', 'probability': 0.0012 }, { 'name': 'Record Player', 'probability': 0.40 }, { 'name': 'Lol Pop', 'probability': 0.10 }, { 'name': 'Hynpo Lollipop', 'probability': 0.10 }, { 'name': 'Party Sparkler', 'probability': 0.10 }, { 'name': 'Skull Flower', 'probability': 0.10 }, { 'name': 'Jelly Bunny', 'probability': 0.0988 }, { 'name': 'Tama Gadget', 'probability': 0.05 }, { 'name': 'Snow Globe', 'probability': 0.05 } ] }, { 'id': 'swisswatch', 'name': 'Swiss Watch Box', 'priceTON': 10.0, 'prizes': [ { 'name': 'Plush Pepe', 'probability': 0.0015 }, { 'name': 'Swiss Watch', 'probability': 0.08 }, { 'name': 'Neko Helmet', 'probability': 0.10 }, { 'name': 'Eternal Rose', 'probability': 0.05 }, { 'name': 'Electric Skull', 'probability': 0.03 }, { 'name': 'Diamond Ring', 'probability': 0.0395 }, { 'name': 'Record Player', 'probability': 0.20 }, { 'name': 'Love Potion', 'probability': 0.20 }, { 'name': 'Top Hat', 'probability': 0.15 }, { 'name': 'Voodoo Doll', 'probability': 0.149 } ] }, { 'id': 'perfumebottle', 'name': 'Perfume Chest', 'priceTON': 20.0, 'prizes': [ { 'name': 'Plush Pepe', 'probability': 0.0018 }, { 'name': 'Perfume Bottle', 'probability': 0.08 }, { 'name': 'Sharp Tongue', 'probability': 0.12 }, { 'name': 'Loot Bag', 'probability': 0.09946 }, { 'name': 'Swiss Watch', 'probability': 0.15 }, { 'name': 'Neko Helmet', 'probability': 0.15 }, { 'name': 'Genie Lamp', 'probability': 0.15 }, { 'name': 'Kissed Frog', 'probability': 0.10 }, { 'name': 'Electric Skull', 'probability': 0.07 }, { 'name': 'Diamond Ring', 'probability': 0.07874 } ] }, { 'id': 'vintagecigar', 'name': 'Vintage Cigar Safe', 'priceTON': 40.0, 'prizes': [ { 'name': 'Plush Pepe', 'probability': 0.002 }, { 'name': 'Perfume Bottle', 'probability': 0.2994 }, { 'name': 'Vintage Cigar', 'probability': 0.12 }, { 'name': 'Swiss Watch', 'probability': 0.12 }, { 'name': 'Neko Helmet', 'probability': 0.10 }, { 'name': 'Sharp Tongue', 'probability': 0.10 }, { 'name': 'Genie Lamp', 'probability': 0.08 }, { 'name': 'Mini Oscar', 'probability': 0.08 }, { 'name': 'Scared Cat', 'probability': 0.05 }, { 'name': 'Toy Bear', 'probability': 0.0486 } ] }, { 'id': 'astralshard', 'name': 'Astral Shard Relic', 'priceTON': 100.0, 'prizes': [ { 'name': 'Plush Pepe', 'probability': 0.0025 }, { 'name': 'Durov\'s Cap', 'probability': 0.09925 }, { 'name': 'Astral Shard', 'probability': 0.10 }, { 'name': 'Precious Peach', 'probability': 0.10 }, { 'name': 'Vintage Cigar', 'probability': 0.12 }, { 'name': 'Perfume Bottle', 'probability': 0.12 }, { 'name': 'Swiss Watch', 'probability': 0.10 }, { 'name': 'Neko Helmet', 'probability': 0.08 }, { 'name': 'Mini Oscar', 'probability': 0.10 }, { 'name': 'Scared Cat', 'probability': 0.08 }, { 'name': 'Loot Bag', 'probability': 0.05 }, { 'name': 'Toy Bear', 'probability': 0.04825 } ] }, { 'id': 'plushpepe', 'name': 'Plush Pepe Hoard', 'priceTON': 200.0, 'prizes': [ { 'name': 'Plush Pepe', 'probability': 0.15 }, { 'name': 'Durov\'s Cap', 'probability': 0.25 }, { 'name': 'Astral Shard', 'probability': 0.60 } ] }, { 'id': 'black', 'name': 'BLACK Singularity', 'isBackgroundCase': True, 'bgImageFilename': 'image-1.png', 'overlayPrizeName': 'Neko Helmet', 'priceTON': 30.0, 'prizes': [ { 'name': 'Plush Pepe', 'probability': 0.001 }, { 'name': 'Durov\'s Cap', 'probability': 0.01 }, { 'name': 'Perfume Bottle', 'probability': 0.05 }, { 'name': 'Mini Oscar', 'probability': 0.04 }, { 'name': 'Scared Cat', 'probability': 0.06 }, { 'name': 'Vintage Cigar', 'probability': 0.07 }, { 'name': 'Loot Bag', 'probability': 0.07 }, { 'name': 'Sharp Tongue', 'probability': 0.08 }, { 'name': 'Genie Lamp', 'probability': 0.08 }, { 'name': 'Swiss Watch', 'probability': 0.10 }, { 'name': 'Neko Helmet', 'probability': 0.15 }, { 'name': 'Kissed Frog', 'probability': 0.10 }, { 'name': 'Electric Skull', 'probability': 0.09 }, { 'name': 'Diamond Ring', 'probability': 0.089} ] } ]
cases_data_backend = []
...
def finalize_slot_prize_pools():
    ...
    for slot_data in slots_data_backend:
        ...
        if slot_data['id'] == 'default_slot':
            ...
            if item_candidates: prob_per_item = remaining_prob_for_items / len(item_candidates);
            ...
            if item_candidates: prob_per_item = remaining_prob_for_items / len(item_candidates)
            for item in item_candidates: temp_pool.append({**item, 'probability': prob_per_item})
        current_total_prob = sum(p_item.get('probability', 0) for p_item in temp_pool)
        if current_total_prob > 0 and abs(current_total_prob - 1.0) > 0.001 :
            ...
            for p_item_norm in temp_pool: p_item_norm['probability'] = p_item_norm.get('probability', 0) / current_total_prob # Renamed p to p_item_norm
...
def populate_initial_data():
    ...
    except Exception as e_populate: # Renamed e
        db.rollback()
        logger.error(f"Error populating initial NFT data: {e_populate}")
    finally:
        db.close()
...
def initial_setup_and_logging():
    ...
    except Exception as e_seed: # Renamed e
        db.rollback()
        logger.error(f"Error seeding Grachev promocode: {e_seed}")
    finally:
        db.close()
...
def validate_init_data(init_data_str: str, bot_token: str) -> dict | None:
    try:
        ...
        if not all(k_check in parsed_data for k_check in ['hash', 'user', 'auth_date']): return None # Renamed k
        ...
        for k_sorted_val in sorted(parsed_data.keys()): data_check_string_parts.append(f"{k_sorted_val}={parsed_data[k_sorted_val][0]}") # Renamed k
    ...
    except Exception as e_validate_init: # Renamed e
        logger.error(f"initData validation error: {e_validate_init}", exc_info=True); return None
...
def get_user_data_api():
    ...
    try:
        ...
        inv = [{"id": item.id, "name": item.nft.name if item.nft else item.item_name_override, "imageFilename": item.nft.image_filename if item.nft else item.item_image_override, "floorPrice": item.nft.floor_price if item.nft else item.current_value, "currentValue": item.current_value, "upgradeMultiplier": item.upgrade_multiplier, "variant": item.variant, "is_ton_prize": item.is_ton_prize, "obtained_at": item.obtained_at.isoformat() if item.obtained_at else None} for item in user.inventory]
...
def open_case_api():
    ...
    try:
        ...
        tcase = next((c_item for c_item in cases_data_backend if c_item['id'] == cid), None)
        ...
        for _ in range(multiplier):
            ...
            for p_info_case in prizes_in_case: # Renamed p_info
                cprob += p_info_case['probability']
                if rv <= cprob: chosen_prize_info = p_info_case; break
            ...
            if not dbnft: logger.error(f"NFT {chosen_prize_info['name']} missing for case open!"); continue
            ...
            except Exception as e_flush_open_case: # Renamed
                db.rollback(); logger.error(f"Error flushing item {dbnft.name}: {e_flush_open_case}"); continue
    ...
    except Exception as e_outer_open_case: # Renamed
        db.rollback(); logger.error(f"Outer error in open_case: {e_outer_open_case}"); return jsonify({"error": "DB error."}), 500
    finally: db.close()
...
def spin_slot_api():
    ...
    try:
        user = db.query(User).filter(User.id == uid).first()
        if not user: return jsonify({"error": "User not found"}), 404
        target_slot = next((s_item for s_item in slots_data_backend if s_item['id'] == slot_id), None)
        ...
        for _ in range(num_reels):
            ...
            for p_info_slot_spin in slot_pool: # Renamed
                cprob += p_info_slot_spin.get('probability', 0)
                if rv <= cprob: landed = p_info_slot_spin; break
        ...
        if num_reels == 3 and len(reel_results_data) == 3:
            ...
            if is_item_win:
                ...
                for i_reel_check in range(1, num_reels): # Renamed
                    if reel_results_data[i_reel_check]['name'] != first_item_name: is_item_win = False; break
            if is_item_win and first_item_name is not None:
                ...
                if db_nft:
                    ...
                    total_value_this_spin += actual_val
                else: logger.error(f"Slot win: NFT {won_item_data['name']} not found in database!")
    ...
    except Exception as e_outer_slot_api_main: # Renamed
        db.rollback(); logger.error(f"Outer error in spin_slot: {e_outer_slot_api_main}", exc_info=True); return jsonify({"error": "DB error during slot spin."}), 500
    finally: db.close()
...
def upgrade_item_api():
    auth = validate_init_data(flask_request.headers.get('X-Telegram-Init-Data'), BOT_TOKEN)
    if not auth: return jsonify({"error": "Auth failed"}), 401
    uid = auth["id"]; data = flask_request.get_json(); iid_upg = data.get('inventory_item_id'); mult_str = data.get('multiplier_str') # Renamed iid
    if not all([iid_upg, mult_str]): return jsonify({"error": "Missing params"}), 400
    try: mult = Decimal(mult_str); iid_upg_int = int(iid_upg) # Renamed iid
    ...
    db = next(get_db())
    try:
        item = db.query(InventoryItem).filter(InventoryItem.id == iid_upg_int, InventoryItem.user_id == uid).first()
        ...
        if random.uniform(0,100) < chances[mult]:
            ...
            db.commit(); return jsonify({"status": "success", "message": f"Upgraded! New value: {new_val:.2f} TON", "item": {"id": item.id, "currentValue": item.current_value, "name": item.nft.name, "upgradeMultiplier": item.upgrade_multiplier, "variant": item.variant }})
        else:
    ...
    except Exception as e_upgrade_api_main: # Renamed
        db.rollback(); logger.error(f"Error during item upgrade: {e_upgrade_api_main}", exc_info=True); return jsonify({"error": "DB error during upgrade."}), 500
    finally: db.close()
...
def convert_to_ton_api():
    auth = validate_init_data(flask_request.headers.get('X-Telegram-Init-Data'), BOT_TOKEN)
    if not auth: return jsonify({"error": "Auth failed"}), 401
    uid = auth["id"]; data = flask_request.get_json(); iid_conv = data.get('inventory_item_id') # Renamed
    if not iid_conv: return jsonify({"error": "ID required"}), 400
    try: iid_conv_int = int(iid_conv) # Renamed
    except ValueError: return jsonify({"error": "Invalid ID"}), 400
    db = next(get_db())
    try:
        user = db.query(User).filter(User.id == uid).first(); item = db.query(InventoryItem).filter(InventoryItem.id == iid_conv_int, InventoryItem.user_id == uid).first()
        ...
        item_name_conv = item.nft.name; db.delete(item); db.commit() # Renamed
        return jsonify({"status": "success", "message": f"{item_name_conv} sold for {val:.2f} TON.", "new_balance_ton": user.ton_balance})
    except Exception as e_convert_api_main: # Renamed
        db.rollback(); logger.error(f"Error converting item to TON: {e_convert_api_main}", exc_info=True); return jsonify({"error": "DB error during conversion."}), 500
    finally: db.close()
...
def sell_all_items_api():
    ...
    try:
        ...
        total_val = sum(Decimal(str(i_to_sell.current_value)) for i_to_sell in items_to_sell) # Renamed
        user.ton_balance = float(Decimal(str(user.ton_balance)) + total_val)
        for i_to_del in items_to_sell: db.delete(i_to_del) # Renamed
    ...
    except Exception as e_sell_all_api_main: # Renamed
        db.rollback(); logger.error(f"Error selling all items: {e_sell_all_api_main}", exc_info=True); return jsonify({"error": "DB error during sell all."}), 500
    finally: db.close()
...
def initiate_deposit_api():
    ...
    try:
        user = db.query(User).filter(User.id == uid).first()
        if not user: return jsonify({"error": "User not found"}), 404
        if db.query(PendingDeposit).filter(PendingDeposit.user_id == uid, PendingDeposit.status == 'pending', PendingDeposit.expires_at > dt.now(timezone.utc)).first(): return jsonify({"error": "You already have an active deposit. Please complete or cancel it first."}), 409
    ...
    except Exception as e_init_dep_api_main: # Renamed
        db.rollback(); logger.error(f"Error initiating deposit: {e_init_dep_api_main}", exc_info=True); return jsonify({"error": "DB error during deposit initiation."}), 500
    finally: db.close()
...
async def check_blockchain_for_deposit(pdep: PendingDeposit, db_sess):
                ...
                if cmt_slice.remaining_bits >= 32 and cmt_slice.load_uint(32) == 0:
                    try:
                        ...
                        if comment_text == pdep.expected_comment:
                            ...
                            if not usr: pdep.status = 'failed'; db_sess.commit(); return {"status": "error", "message": "User associated with deposit not found."}
                            ...
                            if usr.referred_by_id:
                                ...
                                if referrer: referral_bonus = (Decimal(str(pdep.original_amount_ton)) * Decimal('0.10')).quantize(Decimal('0.01'),ROUND_HALF_UP); referrer.referral_earnings_pending = float(Decimal(str(referrer.referral_earnings_pending)) + referral_bonus)
                            pdep.status = 'completed'; db_sess.commit()
                            return {"status": "success", "message": "Deposit confirmed!", "new_balance_ton": usr.ton_balance}
                    except Exception as e_comment_parse_bc: logger.debug(f"Error parsing comment or other issue for tx {tx.hash}: {e_comment_parse_bc}"); pass # Renamed
    ...
    except Exception as e_bc_check_main: logger.error(f"Blockchain check error: {e_bc_check_main}", exc_info=True); return {"status": "error", "message": "Error checking transaction on blockchain."} # Renamed
    finally:
...
def verify_deposit_api():
    ...
    try:
        ...
        result = {}
        try:
            current_loop_verify_dep = asyncio.get_event_loop() # Renamed
            if current_loop_verify_dep.is_running():
                logger.warning("verify_deposit_api: Attempting to run async in a potentially running loop context.")
                new_loop_verify_dep = asyncio.new_event_loop(); asyncio.set_event_loop(new_loop_verify_dep); result = new_loop_verify_dep.run_until_complete(check_blockchain_for_deposit(pdep, db)) # Renamed
            else: result = current_loop_verify_dep.run_until_complete(check_blockchain_for_deposit(pdep, db))
        except RuntimeError as e_runtime_verify: # Renamed
            if "cannot be called from a running event loop" in str(e_runtime_verify) or "no current event loop" in str(e_runtime_verify).lower() or "There is no current event loop in thread" in str(e_runtime_verify):
                logger.info("verify_deposit_api: Creating new event loop for blockchain check.")
                new_loop_runtime_verify = asyncio.new_event_loop(); asyncio.set_event_loop(new_loop_runtime_verify); result = new_loop_runtime_verify.run_until_complete(check_blockchain_for_deposit(pdep, db)) # Renamed
            else: logger.error(f"RuntimeError during async execution in verify_deposit: {e_runtime_verify}", exc_info=True); return jsonify({"status": "error", "message": "Internal server error during verification."}), 500
        except Exception as e_async_run_verify_dep: logger.error(f"Exception running async task in verify_deposit: {e_async_run_verify_dep}", exc_info=True); return jsonify({"status": "error", "message": "Unexpected error during verification process."}), 500 # Renamed
        return jsonify(result)
    except Exception as e_verify_outer_api_main: db.rollback(); logger.error(f"Outer error in verify_deposit: {e_verify_outer_api_main}", exc_info=True); return jsonify({"error": "DB error during deposit verification."}), 500 # Renamed
    finally: db.close()
...
def get_leaderboard_api():
    ...
    try:
        ...
        return jsonify([{"rank": r_idx_leader+1, "name": u_leader_obj.first_name or u_leader_obj.username or f"User_{str(u_leader_obj.id)[:6]}", "avatarChar": (u_leader_obj.first_name or u_leader_obj.username or "U")[0].upper(), "income": u_leader_obj.total_won_ton, "user_id": u_leader_obj.id} for r_idx_leader, u_leader_obj in enumerate(leaders)]) # Renamed
    finally: db.close()
...
def withdraw_referral_earnings_api():
    ...
    try:
        ...
        if user.referral_earnings_pending > 0:
            withdrawn_amount = Decimal(str(user.referral_earnings_pending)); user.ton_balance = float(Decimal(str(user.ton_balance)) + withdrawn_amount); user.referral_earnings_pending = 0.0; db.commit()
    ...
    except Exception as e_withdraw_ref_api_main: db.rollback(); logger.error(f"Error withdrawing referral earnings: {e_withdraw_ref_api_main}", exc_info=True); return jsonify({"error": "DB error during referral withdrawal."}), 500 # Renamed
    finally: db.close()
...
def redeem_promocode_api():
    ...
    try:
        ...
        if promo.activations_left <= 0: return jsonify({"status": "error", "message": "Promocode has expired or reached its activation limit."}), 400
        promo.activations_left -= 1; user.ton_balance = float(Decimal(str(user.ton_balance)) + Decimal(str(promo.ton_amount)))
        db.commit(); return jsonify({"status": "success", "message": f"Promocode '{code_txt}' redeemed successfully! You received {promo.ton_amount:.2f} TON.", "new_balance_ton": user.ton_balance})
    except SQLAlchemyError as e_sql_promo_api: db.rollback(); logger.error(f"SQLAlchemyError redeeming promocode: {e_sql_promo_api}", exc_info=True); return jsonify({"status": "error", "message": "Database error redeeming promocode."}), 500 # Renamed
    except Exception as e_promo_api_main: db.rollback(); logger.error(f"General error redeeming promocode: {e_promo_api_main}", exc_info=True); return jsonify({"status": "error", "message": "An unexpected error occurred."}), 500 # Renamed
    finally: db.close()
...
def withdraw_item_via_tonnel_api_sync_wrapper(inventory_item_id):
    ...
    if not TONNEL_SENDER_INIT_DATA: logger.error("Tonnel: TONNEL_SENDER_INIT_DATA not set."); return jsonify({"status": "error", "message": "Withdrawal service is currently unavailable."}), 500
    db = next(get_db())
    try:
        ...
        if not item_to_withdraw or item_to_withdraw.is_ton_prize: return jsonify({"status": "error", "message": "Item not found or is not withdrawable."}), 404
        ...
        tonnel_result = {}
        try:
            current_loop_tonnel_withdraw = asyncio.get_event_loop() # Renamed
            if current_loop_tonnel_withdraw.is_closed(): current_loop_tonnel_withdraw = asyncio.new_event_loop(); asyncio.set_event_loop(current_loop_tonnel_withdraw)
            tonnel_result = current_loop_tonnel_withdraw.run_until_complete(tonnel_client.send_gift_to_user(gift_item_name=item_name_for_tonnel, receiver_telegram_id=player_user_id))
        except RuntimeError as e_async_loop_tonnel_withdraw: # Renamed
             if "cannot schedule new futures after shutdown" in str(e_async_loop_tonnel_withdraw).lower() or "event loop is closed" in str(e_async_loop_tonnel_withdraw).lower() :
                logger.warning(f"Asyncio loop issue in Tonnel withdrawal, trying new loop: {e_async_loop_tonnel_withdraw}")
                new_loop_tonnel_withdraw = asyncio.new_event_loop(); asyncio.set_event_loop(new_loop_tonnel_withdraw); tonnel_result = new_loop_tonnel_withdraw.run_until_complete(tonnel_client.send_gift_to_user(gift_item_name=item_name_for_tonnel, receiver_telegram_id=player_user_id)) # Renamed
             else: raise
        if tonnel_result and tonnel_result.get("status") == "success":
            val_deducted = Decimal(str(item_to_withdraw.current_value)); player = db.query(User).filter(User.id == player_user_id).with_for_update().first()
    ...
    except Exception as e_withdraw_wrapper_api_main: db.rollback(); logger.error(f"Tonnel withdrawal wrapper exception: {e_withdraw_wrapper_api_main}", exc_info=True); return jsonify({"status": "error", "message": "An unexpected error occurred during withdrawal."}), 500 # Renamed
    finally: db.close()
...
def send_welcome(message):
    ...
    db = next(get_db())
    try:
        user_id = message.chat.id; tg_user_obj = message.from_user; user = db.query(User).filter(User.id == user_id).first(); created_now = False
        if not user:
            created_now = True
            ...
            if len(command_parts) > 1 and command_parts[1].startswith('startapp='):
                ...
                if start_param_value.startswith('ref_'):
                    ...
                    if (created_now or not user.referred_by_id) and user.referral_code != referrer_code :
                        ...
                        if referrer:
                            user.referred_by_id = referrer.id; logger.info(f"User {user_id} referred by {referrer.id} via deep link {referrer_code}.")
                            ...
                            except Exception as e_notify_ref_start: logger.warning(f"Failed to notify referrer {referrer.id}: {e_notify_ref_start}") # Renamed
                        elif not referrer: logger.warning(f"Referral code {referrer_code} not found for any other user, or self-referral attempt by {user_id}.")
        except Exception as e_ref_param_start: logger.error(f"Error processing start parameter for {user_id}: {e_ref_param_start}") # Renamed
        ...
        if created_now or updated_fields or user.referred_by_id:
            try:
                db.commit();
                if created_now: db.refresh(user)
                logger.info(f"User data for {user_id} committed/updated.")
            except Exception as e_commit_user_start_main: db.rollback(); logger.error(f"Error committing user {user_id}: {e_commit_user_start_main}"); user_check_after_fail = db.query(User).filter(User.id == user_id).first(); \
                                                      if not user_check_after_fail and created_now: bot.send_message(message.chat.id, "Sorry, there was an error setting up your profile. Please try /start again."); return # Renamed
        button_mini_app_url = f"https://t.me/{bot.get_me().username}/{MINI_APP_NAME or 'app'}"
        if not MINI_APP_NAME: logger.warning("MINI_APP_NAME environment variable is not set. The Mini App button link might be incorrect.")
        markup = types.InlineKeyboardMarkup(); web_app_info = types.WebAppInfo(url=button_mini_app_url); app_button = types.InlineKeyboardButton(text="🎮 Открыть Pusik Gifts", web_app=web_app_info); markup.add(app_button)
        bot.send_message(message.chat.id, "Добро пожаловать в Pusik Gifts! 🎁\n\nНажмите кнопку ниже, чтобы начать!", reply_markup=markup)
    except Exception as e_start_handler_main_wrapper: logger.error(f"General error in /start handler for chat {message.chat.id}: {e_start_handler_main_wrapper}", exc_info=True); bot.send_message(message.chat.id, "An unexpected error occurred. Please try again later.") # Renamed
    finally: db.close()
...
def run_bot_polling():
    ...
    for i_webhook_attempt_poll in range(3): # Renamed
        try: bot.remove_webhook(); logger.info("Webhook successfully removed."); break
        except Exception as e_webhook_remove_poll: logger.warning(f"Attempt {i_webhook_attempt_poll+1} to remove webhook failed: {e_webhook_remove_poll}"); \
                                               if i_webhook_attempt_poll < 2: time.sleep(2) # Renamed
    while bot_polling_started:
        try:
            logger.info("Bot calling infinity_polling..."); bot.infinity_polling(logger_level=logging.INFO, skip_pending=True, timeout=60, long_polling_timeout=30)
            logger.info("infinity_polling finished a cycle or stopped.") 
        except telebot.apihelper.ApiTelegramException as e_api_poll_run: logger.error(f"Telegram API Exception in polling: Code {e_api_poll_run.error_code} - {e_api_poll_run.description}", exc_info=False); \
                                                                      if e_api_poll_run.error_code == 401 or e_api_poll_run.error_code == 409: logger.error("CRITICAL: Bot token invalid or conflict. Stopping polling."); bot_polling_started = False; \
                                                                      else: time.sleep(30) # Renamed
        except ConnectionError as e_conn_poll_run: logger.error(f"Network ConnectionError in polling: {e_conn_poll_run}", exc_info=False); time.sleep(60) # Renamed
        except Exception as e_generic_poll_run: logger.error(f"Unexpected critical error in polling: {type(e_generic_poll_run).__name__} - {e_generic_poll_run}", exc_info=True); time.sleep(60) # Renamed
